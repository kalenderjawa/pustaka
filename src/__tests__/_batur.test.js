...
describe('Batur Testing', () => {
  ...
  test('Year below minimum range (< 1867)', () => {
    ...
    expect(periksaBatasan(1800)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(1000)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(0)).toEqual({ status: false, message: 'out of scoope' });
  });

  test('Year above maximum range (> 2106)', () => {
    expect(periksaBatasan(2107)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(2200)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(3000)).toEqual({ status: false, message: 'out of scoope' });
  });
  ...
  test('Negative years', () => {
    expect(periksaBatasan(-1)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(-100)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(-1867)).toEqual({ status: false, message: 'out of scoope' });
  });

  test('Decimal years (non-integer)', () => {
    expect(periksaBatasan(1900.5)).toEqual({ status: true, message: '' });
    expect(periksaBatasan(1866.9)).toEqual({ status: false, message: 'out of scoope' });
    expect(periksaBatasan(2106.1)).toEqual({ status: false, message: 'out of scoope' });
  });
});